...
class PllPhase270Tests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("240_pll_phase_270")
        self.assertEqual(policy.clock_evidence_names, ("meter.refclk",))
        self.assertEqual(
            dict(policy.additional_clocks_mhz),
            {"meter.testclk": 25.0, "phase270": 25.0},
        )
        self.assertEqual(dict(policy.required_synth_cells), {"altera_pll": 1})
        rtl = (ROOT / "experiments/240_pll_phase_270/rtl/top.v").read_text()
        policy.validate_source_text("experiments/240_pll_phase_270/rtl/top.v", rtl)
        ...
        self.assertIn('.phase_shift1("30000 ps")', rtl)
        self.assertIn("16'hD71B", rtl)
        self.assertIn("always @(posedge phase0) begin", rtl)
        self.assertIn("always @(posedge phase270) begin", rtl)
        self.assertIn("phase0_alive", rtl)
        self.assertIn("phase270_alive", rtl)
    ...
    def test_both_phase_clocks_are_required(self):
        ...
        clk270 = {"constraint": 25, "achieved": 240}
        cases = [
            ({"meter.refclk": ref, "meter.testclk": clk0, "phase270": clk270}, True),
            ({"meter.refclk": ref, "meter.testclk": clk0}, False),
            ({"meter.refclk": ref, "phase270": clk270}, False),
        ]
