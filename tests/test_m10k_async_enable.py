...
RTL = ROOT / "experiments/820_m10k_async_enable/rtl/top.v"
PROBE = ROOT / "experiments/820_m10k_async_enable/hardware/probe.sh"


class M10kAsyncEnableTests(unittest.TestCase):
    def test_sources_exist(self) -> None:
        for relative in (
            "rtl/top.v",
            "sim/tb.cpp",
            "sim/m10k_async_enable_model.v",
            "expected.md",
            "hardware/probe.sh",
        ):
            path = ROOT / "experiments/820_m10k_async_enable" / relative
            self.assertTrue(path.is_file(), path)

    def test_primitive_omits_read_enable_and_second_clock(self) -> None:
        rtl = RTL.read_text(encoding="utf-8")
        self.assertIn("16'hD42F", rtl)
        self.assertIn(".B1EN(1'b1)", rtl)
        self.assertNotIn(".CLK2", rtl)
        self.assertNotIn(".CFG_ASYNC_READ", rtl)
        self.assertNotIn("altera_pll", rtl)
        policy = policy_for("820_m10k_async_enable")
        policy.validate_source_text("experiments/820_m10k_async_enable/rtl/top.v", rtl)
