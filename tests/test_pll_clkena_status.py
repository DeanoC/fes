...
class PllClkenaStatusTests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("390_pll_clkena_status")
        rtl = (ROOT / "experiments/390_pll_clkena_status/rtl/top.v").read_text()
        policy.validate_source_text("experiments/390_pll_clkena_status/rtl/top.v", rtl)
        self.assertIn(".enaout(enable_status)", rtl)
        self.assertIn("sampled_status", rtl)
        self.assertIn('.ena_register_power_up("low")', rtl)
        self.assertIn("16'hD72A", rtl)
