...
(`fes.coleco`). Its normal library launch uses the registered 2,299-byte
controller diagnostic through the existing development-media path. The asset
...
`internal/coremedia/assets/coleco-controllers.{hex,LICENSE}`. The diagnostic
...
explicit `core-media` operation remains available for alternate media; media
is not yet selectable as a separate library entry. No default ZX81 tape is
registered until its source asset and target behavior have been validated.
