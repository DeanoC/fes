...
| `fes.sg1000` | 1.1.0 | Package-only | `fes.simple-computer` 1.0 | `fes.keyboard` 1.0, `fes.video.fixed-720p60` 1.0 | — | SG-1000 slice. Exact 16 KiB `cartridge-rom` linked before download at `0x0000`; pad shorter fixed-map images with `0xff`. 1 KiB RAM at `0xc000`, joysticks on `0xdc`/`0xdd`. No PSG or startup media blob. Lock `toolchains/registered-memory.lock`. |
