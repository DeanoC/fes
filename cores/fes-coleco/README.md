...
| Route pressure | The reproducible passing settings are device `5CSEBA6U23I7`, seed 3, `router1`, no `--tmg-ripup`, and a 74.25 MHz request. Timing-driven rip-up was slower on the packed netlist and regressed a passing `clk_sys` route; keep it disabled until nextpnr reproduces a better result. |
| Relocated Quartus/Icarus probe | The local Icarus wrapper needs `IVERILOG_BASE` set to its actual `.../usr/lib/x86_64-linux-gnu/ivl` directory; use `QUARTUS_ROOTDIR=/home/deano/intelFPGA_lite/17.0/quartus`. The runner uses Icarus for the unmodified Intel `altera_mf.v` model because Verilator rejects the model's `i_good_to_write_a2`/`i_good_to_write_b2` feedback constructs. |
