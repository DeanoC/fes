...
func guardKitLease(next http.Handler, token string, manager *internallease.Manager) http.Handler {
	...
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		switch r.URL.Path {
		case "/v1/development/recover-idle":
			// A blocked or free lease has no owner token. Idle recovery must
			// still be reachable so an operator can LoadIdle without a board reboot.
			state := manager.Status().State
			if state == "blocked" || state == "free" {
				next.ServeHTTP(w, r)
				return
			}
			guarded.ServeHTTP(w, r)
		case "/v1/development/media-stream", "/v1/development/media", "/v1/development/live-media", "/v1/development/clear-media", "/v1/development/firmware", "/v1/library/core/load", "/v1/library/core/compose", "/v1/library/core/settings", "/v1/stop", "/v1/development/rbf", "/v1/development/core", "/v1/development/reboot", "/v1/kit/debug/snapshot-before-reboot", "/v1/input/attach", "/v1/input/detach", "/v1/input/stream", "/v1/cast/start", "/v1/cast/stop", "/v1/update/stage", "/v1/update/activate", "/v1/update/rollback", "/v1/update/confirm":
			guarded.ServeHTTP(w, r)
		default:
			next.ServeHTTP(w, r)
		}
