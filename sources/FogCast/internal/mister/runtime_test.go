...
func TestWriteAtomicDevelopmentRBFReplacesOnlyACompleteFile(t *testing.T) {
	...
	if err := mister.WriteAtomicDevelopmentRBF(path, 8, bytes.NewReader([]byte("short"))); err == nil {
	...
	if err := mister.WriteAtomicDevelopmentRBF(path, int64(len(want)), bytes.NewReader(want)); err != nil {
		t.Fatal(err)
	}
...
func TestWriteAtomicDevelopmentRBFRejectsTrailingByteWithoutReplacingInstalledFile(t *testing.T) {
	...
	if err := mister.WriteAtomicDevelopmentRBF(path, 3, bytes.NewReader([]byte("rbf!"))); err == nil {
...
func TestWriteAtomicDevelopmentRBFRejectsTrailingReadErrorWithoutReplacingInstalledFile(t *testing.T) {
	...
	if err := mister.WriteAtomicDevelopmentRBF(path, 3, content); err == nil {
