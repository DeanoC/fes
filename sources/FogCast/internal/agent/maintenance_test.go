...
func TestUpdateMaintenanceRetainsLaunchExclusionAndRawIdle(t *testing.T) {
	...
	c := agent.New(r, core.DefaultRegistry(), time.Second, time.Second)
	...
	if _, e := c.Launch(context.Background(), protocol.LaunchRequest{}); e == nil || e.Code != protocol.CodeBusy {
...
func TestUpdateMaintenanceRejectsFailedSaveAndNonNativeIdle(t *testing.T) {
	...
	legacy := agent.New(&fakeRuntime{health: protocol.Health{Ready: true}}, core.DefaultRegistry(), time.Second, time.Second)
