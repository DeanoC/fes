...
# loading build/oss/220_pll_phase/top.rbf. This script does not program or claim hardware.
# It checks 25 MHz and 0-to-90 capture, not analog phase accuracy.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55065 ]; then # 0xD719
        echo "FAIL: 0/90 phase PLL diagnostic signature missing: $1" >&2
        exit 1
    fi
}
set_reset() {
...
set_launch() {
    ...
    while :; do
        ...
        polls=$((polls + 1))
        [ "$polls" -lt 100 ] || { echo "FAIL: 0-to-90 capture timeout: $status" >&2; exit 1; }
        sleep 0.02
    done
}
measure() {
