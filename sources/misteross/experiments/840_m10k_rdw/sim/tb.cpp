...
namespace {
constexpr uint32_t kSignature = 0xd8400000U;
...
int main(int argc, char **argv) {
    ...
    gpo(top) = kArm;
    ticks(top, 4);
    gpo(top) = (1u << 31) | kEn | ((written & 0xfffffu) << 9) | addr;
    ticks(top, 16);
    if (!expect_read(top, addr, written, "write-through"))
        return EXIT_FAILURE;
    if (!expect_read(top, 1u, initial_word(1u), "neighbour"))
        return EXIT_FAILURE;
    std::cout << "PASS: TDP A-port NEW_DATA write-through of INIT and a write\n";
    return EXIT_SUCCESS;
}
