...
# loading build/oss/830_pll_frac_27/top.rbf. This script does not program or claim hardware.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55335 ]; then # 0xD827
        echo "FAIL: 27 MHz fractional PLL diagnostic signature missing: $1" >&2
        exit 1
    fi
}
for trial in 1 2 3; do
    ...
    # 27 * 4096 / 50 = 2211.84; accept window-endpoint slop of one count.
    if [ "$((status & 12288))" -ne 8192 ] || [ "$count" -lt 2210 ] || [ "$count" -gt 2213 ]; then
        echo "FAIL: trial=$trial count=$count status=$status (expected 2212 +/- 2, locked, no sampled lock loss)" >&2
        exit 1
    fi
