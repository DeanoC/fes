...
# loading build/oss/720_m10k_aclr_infer/top.rbf. This script does not program or claim hardware.
# GPO[5] is inferred ACLR1 and is cleared from the table address, so probes below use address 7.
set -eu
signature=54312
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing inferred M10K ACLR signature $status" >&2; exit 1;
    }
    actual=$((status & 65535))
    expected=$(($1))
    [ "$actual" -eq "$expected" ] || {
