...
int main(int argc, char **argv) {
    ...
    auto step = [&]() {
        ...
        dut.FPGA_CLK1_50 = 1;
        dut.eval();
        assert((gp.gpi >> 16) == 0xd717);
    };
    auto select = [&](unsigned channel) {
        gp.gpo = (gp.gpo & ~8U) | (channel << 3);
        for (int i = 0; i < 16; ++i)
            step();
    };
    ...
    for (unsigned channel = 0; channel < 2; ++channel) {
        select(channel);
        unsigned lo = channel ? 2013U : 1006U;
        unsigned hi = channel ? 2014U : 1007U;
    ...
    std::cout << "PASS: dual 12.288/24.576 MHz meters; reset zero, released 1006-1007/2013-2014\n";
}
