...
# loading build/oss/390_pll_clkena_status/top.rbf. This script does not program or claim hardware.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55082 ]; then # 0xD72A
        echo "FAIL: clock-enable status PLL diagnostic signature missing: $1" >&2
        exit 1
    fi
}
set_enable() {
