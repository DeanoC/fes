#!/bin/sh
# Empty ZX81 socket: signature 0xD904 and vacant plug reads. Hold the kit.py lease.
set -eu
signature=55556
status=$(busybox devmem 0xFF706014 32)
[ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
    echo "FAIL: missing 904 socket signature $status" >&2
    exit 1
}
...
echo 'PASS: 904 empty socket reads 0, plug_addr follows GPO'
