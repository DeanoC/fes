# 130 40 MHz PLL-clocked DSP product

`make sim EXP=130_pll_dsp_40` tests HPS peek/poke of an eight-by-eight unsigned
product computed on a 50-to-40 MHz integer PLL output. `make oss EXP=130_pll_dsp_40`
produces compressed `build/oss/130_pll_dsp_40/top.rbf`. It requires one
`altera_pll`, one `MISTRAL_MUL9X9`, one HPS GP interface, no memory, and passing
50/40 MHz timing. Quartus comparison is not implemented.

The PLL uses the checked configurable integer profile: PIN_V11 50 MHz → 40 MHz,
direct mode, integer operation, zero phase, 50% duty, reset held low. Operands
cross from the 50 MHz HPS domain into `clk40`; the product crosses back. Hold
GPO operands until both GPI bytes have been read. Simulation models only a
digital toggling clock and treats lock as the inverse of reset.

| Register bits | Meaning |
| --- | --- |
| GPI [31:16] | Signature `0xDC40` |
