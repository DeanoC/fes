...
# loading build/oss/300_pll_ref25/top.rbf. Requires a physical 25 MHz V11 clock. This script does not
# program or claim hardware.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55073 ]; then # 0xD721
        echo "FAIL: 25 MHz-ref PLL diagnostic signature missing: $1" >&2
        exit 1
    fi
}
set_reset() {
