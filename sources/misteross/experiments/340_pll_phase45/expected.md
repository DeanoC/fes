# 340 four-output 50 MHz 45-degree PLL

`make sim EXP=340_pll_phase45` tests reset/relock and frequency meters on four
50 MHz outputs of one integer `altera_pll` at 0°/90°/270°/315°.
`make oss EXP=340_pll_phase45` produces compressed
`build/oss/340_pll_phase45/top.rbf`. It requires one PLL, one HPS GP, no
...
with `phase_shift` `0 ps` / `5000 ps` / `15000 ps` / `17500 ps` from the
compiler `quadMixed` fixture. GPO bits 4:3 select which meter is visible.
Simulation models digital 50 MHz clocks; analog 45° steps are not modeled.
Kit measurement proves four 50 MHz clocks exist, not analog phase accuracy.

| Register bits | Meaning |
| --- | --- |
| GPI [31:16] | Signature `0xD725` |
...
| GPO [4:3] | Meter select: 0=0°, 1=90°, 2=270°, 3=315° |
