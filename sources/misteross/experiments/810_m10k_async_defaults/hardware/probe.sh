...
# loading build/oss/810_m10k_async_defaults/top.rbf. This script does not program or claim hardware.
set -eu
signature=54318
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing async-default M10K signature $status" >&2; exit 1;
    }
    actual=$((status & 65535))
    expected=$(($1))
    [ "$actual" -eq "$expected" ] || {
