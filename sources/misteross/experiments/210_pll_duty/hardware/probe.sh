...
# loading build/oss/210_pll_duty/top.rbf. This script does not program or claim hardware.
# It checks frequency and mixed-edge capture, not pulse width.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55064 ]; then # 0xD718
        echo "FAIL: 25% duty PLL diagnostic signature missing: $1" >&2
        exit 1
    fi
}
set_reset() {
...
set_launch() {
    ...
    while :; do
        ...
        polls=$((polls + 1))
        [ "$polls" -lt 100 ] || { echo "FAIL: mixed-edge capture timeout: $status" >&2; exit 1; }
        sleep 0.02
    done
}
measure() {
