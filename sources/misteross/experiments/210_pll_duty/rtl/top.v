module top(input wire FPGA_CLK1_50);
    ...
    ) pll (.refclk(FPGA_CLK1_50), .rst(reset_sync), .outclk(duty_clock), .locked(locked));
    always @(posedge duty_clock)
        launch <= gpo[0];
    always @(negedge duty_clock)
        capture <= launch;
    cyclonev_hps_interface_mpu_general_purpose hps_gp (.gp_in(gpi), .gp_out(gpo));
    pll_meter meter (
        .refclk(FPGA_CLK1_50), .testclk(duty_clock), .locked(locked), .request(gpo[1]),
        .busy(busy), .done(done), .lost_lock(lost_lock),
        .lock_status(lock_status), .result(result)
    );
    assign gpi = {16'hD718, busy, done, lock_status, lost_lock, reset_sync, 1'b0, launch, capture,
