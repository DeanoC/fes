...
int main(int argc, char **argv) {
    ...
    auto step = [&]() {
        ...
        dut.FPGA_CLK1_50 = 1;
        dut.eval();
        assert((gp.gpi >> 16) == 0xd71d);
    };
    auto select = [&](unsigned channel) {
        gp.gpo = (gp.gpo & ~24U) | (channel << 3);
        for (int i = 0; i < 16; ++i)
            step();
    };
    ...
    // Channels 2/3 are 100/75 MHz on hardware; this digital model is 50/37.5 MHz.
    const unsigned lo[4] = {2047U, 4095U, 4095U, 3071U};
    const unsigned hi[4] = {2049U, 4097U, 4097U, 3073U};
    for (unsigned channel = 0; channel < 4; ++channel) {
        ...
        for (int trial = 0; trial < 3; ++trial) {
            ...
            measure(true, 0, 0);
            reset(false);
            measure(false, lo[channel], hi[channel]);
        }
    }
    std::cout << "PASS: quad meters; reset zero, released 2048/4096 and digital 100/75 MHz stand-ins\n";
}
