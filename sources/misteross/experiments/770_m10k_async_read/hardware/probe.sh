...
# loading build/oss/770_m10k_async_read/top.rbf. This script does not program or claim hardware.
set -eu
signature=54315
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing async-read M10K signature $status" >&2; exit 1;
    }
    actual=$((status & 65535))
    expected=$(($1))
    [ "$actual" -eq "$expected" ] || {
