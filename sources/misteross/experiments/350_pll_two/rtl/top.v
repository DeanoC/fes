module top(input wire FPGA_CLK1_50);
    wire integer_clock;
    wire fractional_clock;
    wire integer_locked;
    wire fractional_locked;
    wire [31:0] gpo;
    wire [31:0] gpi;
    ...
    reg reset_sync = 0;
    (* keep = "true" *) reg integer_alive = 0;
    (* keep = "true" *) reg fractional_alive = 0;
    ...
    altera_pll #(
        ...
        .refclk(FPGA_CLK1_50),
        .rst(reset_sync),
        .outclk(integer_clock),
        .locked(integer_locked)
    );
    altera_pll #(
        .reference_clock_frequency("50.0 MHz"),
        .number_of_clocks(1),
        ...
        .refclk(FPGA_CLK1_50),
        .rst(reset_sync),
        .outclk(fractional_clock),
        .locked(fractional_locked)
    );
    always @(posedge integer_clock) integer_alive <= ~integer_alive;
    always @(posedge fractional_clock) fractional_alive <= ~fractional_alive;
    cyclonev_hps_interface_mpu_general_purpose hps_gp (.gp_in(gpi), .gp_out(gpo));
    pll_meter meter (
        .refclk(FPGA_CLK1_50), .testclk(integer_clock), .locked(integer_locked),
        .request(gpo[1]), .busy(busy0), .done(done0), .lost_lock(lost0),
        .lock_status(lock0), .result(result0)
    );
    pll_meter meter1 (
        .refclk(FPGA_CLK1_50), .testclk(fractional_clock), .locked(fractional_locked),
        .request(gpo[1]), .busy(busy1), .done(done1), .lost_lock(lost1),
        .lock_status(lock1), .result(result1)
    );
    wire [15:0] selected = gpo[3] ? result1 : result0;
    wire selected_busy = gpo[3] ? busy1 : busy0;
    wire selected_done = gpo[3] ? done1 : done0;
    wire selected_lock = gpo[3] ? lock1 : lock0;
    wire selected_lost = gpo[3] ? lost1 : lost0;
    assign gpi = {16'hD726, selected_busy, selected_done, selected_lock, selected_lost,
