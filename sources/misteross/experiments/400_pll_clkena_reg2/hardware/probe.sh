...
# loading build/oss/400_pll_clkena_reg2/top.rbf. This script does not program or claim hardware.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55083 ]; then # 0xD72B
        echo "FAIL: double-register clock-enable PLL diagnostic signature missing: $1" >&2
        exit 1
    fi
}
set_enable() {
    ...
    while :; do
        ...
        if [ "$(((status >> 10) & 1))" -eq "$1" ] && [ "$(((status >> 9) & 1))" -eq "$1" ]; then
            break
        fi
        polls=$((polls + 1))
        [ "$polls" -lt 100 ] || { echo "FAIL: enable/status timeout: $status" >&2; exit 1; }
        sleep 0.02
    done
    sleep 0.02
}
