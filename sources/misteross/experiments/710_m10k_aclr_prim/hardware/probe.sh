...
# loading build/oss/710_m10k_aclr_prim/top.rbf. This script does not program or claim hardware.
# GPO[5] is ACLR1 and is cleared from the table address, so probes below use address 7.
set -eu
signature=54311
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing M10K primitive ACLR signature $status" >&2; exit 1;
    }
    actual=$((status & 65535))
    expected=$(($1))
    [ "$actual" -eq "$expected" ] || {
