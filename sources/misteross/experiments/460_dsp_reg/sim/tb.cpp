...
namespace {
constexpr uint32_t kSignature = 0xd6160000U;
...
int main(int argc, char **argv) {
    ...
    const struct { uint8_t left, right; uint16_t wide; const char *label; } cases[] = {
    ...
    for (const auto &item : cases) {
        if (!expect_product(top, item.left, item.right, item.wide, item.label)) return EXIT_FAILURE;
    }
    std::cout << "PASS: HPS peek/poke of registered M18 product verified\n";
    return EXIT_SUCCESS;
}
