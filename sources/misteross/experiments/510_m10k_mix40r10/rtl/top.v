// Mixed-width M10K SDP: 256x40 write, 1024x10 read. 50 MHz write, gated 25 MHz read.
module top #(
    parameter [15:0] SIGNATURE = 16'hD41B
) (
    input wire FPGA_CLK1_50
);
    localparam integer UNIT = 10;
    localparam integer WLANES = 4;
    localparam integer RLANES = 1;
    localparam integer WA = 8;
    ...
    generate
        for (lane = 0; lane < WLANES; lane = lane + 1) begin: write_lane
            ...
            always @(posedge FPGA_CLK1_50)
                if (we)
                    mem[{wa, INDEX}] <= seed ^ (lane[UNIT-1:0] * 10'h093);
        end
    ...
    wire [63:0] windowed = {54'd0, q} >> {gp_out[27:26], 4'b0};
    reg [15:0] sampled = 16'd0;
    always @(posedge FPGA_CLK1_50)
        sampled <= {6'b0, windowed[9:0]};
