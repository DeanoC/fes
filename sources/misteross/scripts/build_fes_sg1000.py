#!/usr/bin/env python3
"""Build and seal the Quartus bring-up FES SG-1000 package."""
...
TARGET = "5CSEBA6U23I7"
TOP = "top"
OUTPUT_RELATIVE = Path("build/fes-sg1000-quartus")
RECIPE = "scripts/build_fes_sg1000.py"
...
class BuildError(ValueError):
    """Raised when the Quartus SG-1000 recipe cannot produce a sealed package."""
...
def write_project(root: Path, output: Path, build_id: str) -> Path:
    ...
    if output != root / OUTPUT_RELATIVE:
        raise BuildError(f"FES SG-1000 Quartus output must be {root / OUTPUT_RELATIVE}")
    ...
    evidence.update(
        {
            "build_id": build_id,
            "device": TARGET,
            "inputs": {(source_provenance.context(root).qualify(relative) if require_clean else relative): _sha256(root / relative)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"build-fes-sg1000-quartus: {exc}", file=sys.stderr)
        return 1
    return 0
