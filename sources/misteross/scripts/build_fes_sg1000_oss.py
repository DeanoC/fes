#!/usr/bin/env python3
"""Build and seal the OSS nextpnr/Mistral FES SG-1000 package."""
...
SG1000_TOOLCHAIN_CONFIGURATION = (
...
def _authenticate_sg1000_tools(root: Path, cache_root: Path | None = None):
    return _authenticate_tools(
        root,
        lock_path=root / SG1000_TOOLCHAIN_LOCK,
        toolchain_root=root / SG1000_TOOLCHAIN_ROOT,
        expected_commits=SG1000_TOOL_COMMITS,
        expected_configuration={"nextpnr": SG1000_TOOLCHAIN_CONFIGURATION},
        gpu_router=SG1000_GPU_ROUTER,
        hip_architectures=SG1000_GPU_ARCHITECTURES,
...
def build_commands(
    ...
    if output != root / OUTPUT_RELATIVE:
        raise BuildError(f"FES SG-1000 OSS output must be {root / OUTPUT_RELATIVE}")
...
def validate_synth_evidence(output: Path) -> dict:
    ...
    if counts.get("MISTRAL_M10K", 0) + counts.get("MISTRAL_M10K_TDP", 0) < 1:
        raise BuildError("synthesis must map SG-1000 RAM onto MISTRAL_M10K")
...
def build(root: Path = ROOT, package_store: Path | None = None, *, cache_root: Path | None = None, identity_version: int = 1, gpu_device: int = 0) -> Path:
    ...
    if package_store != root / "build/packages":
        raise BuildError(f"FES SG-1000 package store must be {root / 'build/packages'}")
    ...
    try:
        ...
        final_tools = _authenticate_sg1000_tools(root, cache_root=cache_root)
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"build-fes-sg1000-oss: {exc}", file=sys.stderr)
        return 1
    return 0
