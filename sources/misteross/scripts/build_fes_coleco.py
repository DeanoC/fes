#!/usr/bin/env python3
"""Build and seal the Quartus bring-up FES COLECO package."""
...
class BuildError(ValueError):
    """Raised when the Quartus COLECO recipe cannot produce a sealed package."""
...
def write_project(root: Path, output: Path, build_id: str) -> Path:
    ...
    if output != root / OUTPUT_RELATIVE:
        raise BuildError(f"FES COLECO Quartus output must be {root / OUTPUT_RELATIVE}")
    ...
    rom_dir = project / "cores" / "fes-coleco" / "rtl"
    rom_dir.mkdir(parents=True)
    shutil.copyfile(_regular_input(root, RESET_ROM_HEX), rom_dir / "coleco_reset_rom.hex")
    shutil.copyfile(_regular_input(root, RESET_ROM_MIF), rom_dir / "coleco_reset_rom.mif")
    return project
...
def build(
    ...
    *,
    ...
    if package_store != root / "build/packages":
        raise BuildError(f"FES COLECO package store must be {root / 'build/packages'}")
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    try:
        ...
        print(f"build-fes-coleco-quartus: {exc}", file=sys.stderr)
        return 1
    return 0
