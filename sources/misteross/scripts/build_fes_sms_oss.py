...
TOP = "top"
ROUTER = "gpu"
# Seed 3 / HeAP 1000 meets both clocks on the linked-ROM diagnostic netlist.
# A sealed BUILD_ID changes placement, so retain the bounded fallbacks.
PLACER_SEEDS = (3, 10, 12, 2, 4, 5, 7, 1)
SEED = PLACER_SEEDS[0]
...
SMS_TOOLCHAIN_LOCK = "toolchains/registered-memory.lock"
...
RTL_SOURCES = (
    ...
    "cores/fes-sms/rtl/sms_rom_link.v",
...
def build_commands(
    ...
    yosys_program = (
        f"read_verilog -sv -DTV80_REFRESH=1 -DFES_SMS_OSS=1 -DFES_SMS_ROM_LINK=1 -DFES_COLECO_OSS=1 "
        f"-I cores/fes-sms/generated {sources}; "
...
def _manifest(
    ...
    fields = {
        ...
        "core": {
            ...
            "description": "Fixed-map Master System with a linked 32 KiB cartridge ROM",
            "version": "1.3.0",
        },
        "target": {
            "platform": "de10_nano",
...
def build(root: Path = ROOT, package_store: Path | None = None, *, cache_root: Path | None = None, identity_version: int = 2, gpu_device: int = 0) -> Path:
    ...
    try:
        ...
        evidence["rom"] = {"id": "cartridge-rom", "role": "cartridge", "source_size": 32768,
