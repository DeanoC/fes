...
from scripts import source_provenance, export_core_package as exporter
...
PRODUCERS = [importlib.import_module('scripts.build_fes_' + name)
             for name in ('coleco', 'sms', 'sg1000', 'zx81')]
...
class SourceProvenanceTests(unittest.TestCase):
    ...
    def test_oracles_export_qualified_records(self):
    ...
    def test_wrong_nested_path_symlink_and_embedded_checkout_rejected(self):
        ...
        with self.assertRaisesRegex(ValueError, 'exact sources/misteross'):
            source_provenance.context(wrong)
        link = repo / 'link'
        link.symlink_to(module, target_is_directory=True)
        with self.assertRaisesRegex(ValueError, 'symlink'):
            source_provenance.context(link)
        git(module, 'init', '-q')
        with self.assertRaisesRegex(ValueError, 'exact sources/misteross'):
            source_provenance.context(module)
