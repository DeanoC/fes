...
class PllRef100Tests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("310_pll_ref100")
        self.assertEqual(policy.clock_mhz, 100.0)
        ...
        rtl = (ROOT / "experiments/310_pll_ref100/rtl/top.v").read_text()
        policy.validate_source_text("experiments/310_pll_ref100/rtl/top.v", rtl)
        self.assertIn('.reference_clock_frequency("100.0 MHz")', rtl)
        self.assertIn('.number_of_clocks(3)', rtl)
        self.assertIn('.output_clock_frequency2("100.0 MHz")', rtl)
        self.assertIn("16'hD722", rtl)
        sdc = (ROOT / "experiments/310_pll_ref100/clocks.sdc").read_text()
        self.assertIn("-period 10.000", sdc)
    ...
    def test_all_output_clocks_are_required(self):
        policy = policy_for("310_pll_ref100")
        ref = {"constraint": 100, "achieved": 200}
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            for clocks, passing in cases:
                with self.subTest(clocks=clocks):
                    ...
                    if passing:
                        self.assertEqual(
                            _timing(path, 100, policy.clock, policy),
                            ("meter.refclk", 200),
                        )
                    else:
