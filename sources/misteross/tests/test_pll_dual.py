...
class PllDualTests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("170_pll_dual")
        self.assertEqual(policy.clock_evidence_names, ("meter.refclk",))
        self.assertEqual(
            dict(policy.additional_clocks_mhz),
            {"clocks[0]": 25.0, "clocks[1]": 40.0},
        )
        self.assertEqual(dict(policy.required_synth_cells), {"altera_pll": 1})
        rtl = (ROOT / "experiments/170_pll_dual/rtl/top.v").read_text()
        policy.validate_source_text("experiments/170_pll_dual/rtl/top.v", rtl)
        self.assertIn('.number_of_clocks(2)', rtl)
        self.assertIn('.output_clock_frequency0("25.0 MHz")', rtl)
        self.assertIn('.output_clock_frequency1("40.0 MHz")', rtl)
        self.assertIn("16'hD714", rtl)
    ...
    def test_three_clock_domains_required(self):
        ...
        clk40 = {"constraint": 40, "achieved": 300}
        cases = [
            ({"meter.refclk": ref, "clocks[0]": clk25, "clocks[1]": clk40}, True),
            ({"meter.refclk": ref, "clocks[0]": clk25}, False),
            ({"meter.refclk": ref, "clocks[1]": clk40}, False),
            (
                {
                    "meter.refclk": ref,
                    "clocks[0]": clk25,
                    "clocks[1]": {"constraint": 40, "achieved": 39},
                },
                False,
            ),
        ]
