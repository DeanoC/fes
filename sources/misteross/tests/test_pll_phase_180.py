...
class PllPhase180Tests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("230_pll_phase_180")
        self.assertEqual(policy.clock_evidence_names, ("meter.refclk",))
        self.assertEqual(
            dict(policy.additional_clocks_mhz),
            {"meter.testclk": 25.0, "phase180": 25.0},
        )
        self.assertEqual(dict(policy.required_synth_cells), {"altera_pll": 1})
        rtl = (ROOT / "experiments/230_pll_phase_180/rtl/top.v").read_text()
        policy.validate_source_text("experiments/230_pll_phase_180/rtl/top.v", rtl)
        ...
        self.assertIn('.phase_shift1("20000 ps")', rtl)
        self.assertIn("16'hD71A", rtl)
        self.assertIn("always @(posedge phase0) begin", rtl)
        self.assertIn("always @(posedge phase180) begin", rtl)
        self.assertIn("phase0_alive", rtl)
        self.assertIn("phase180_alive", rtl)
    ...
    def test_both_phase_clocks_are_required(self):
        ...
        clk180 = {"constraint": 25, "achieved": 240}
        cases = [
            ({"meter.refclk": ref, "meter.testclk": clk0, "phase180": clk180}, True),
            ({"meter.refclk": ref, "meter.testclk": clk0}, False),
            ({"meter.refclk": ref, "phase180": clk180}, False),
        ]
