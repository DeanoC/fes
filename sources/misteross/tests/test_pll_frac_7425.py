...
class PllFrac7425Tests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("610_pll_frac_7425")
        self.assertEqual(policy.clock_evidence_names, ("meter.refclk",))
        self.assertEqual(dict(policy.additional_clocks_mhz), {"fractional_clock": 74.25})
        self.assertEqual(dict(policy.required_synth_cells), {"altera_pll": 1})
        rtl = (ROOT / "experiments/610_pll_frac_7425/rtl/top.v").read_text()
        policy.validate_source_text("experiments/610_pll_frac_7425/rtl/top.v", rtl)
        self.assertIn('.output_clock_frequency0("74.25 MHz")', rtl)
        self.assertIn('.fractional_vco_multiplier("true")', rtl)
        self.assertIn("16'hD742", rtl)
    ...
    def test_picosecond_constraint_quantization_is_accepted(self):
        ...
        generated = {"constraint": 74.25006866, "achieved": 343.879}
                ...
                json.dumps(
                    {
                        "fmax": {
                            "meter.refclk": ref,
                            "fractional_clock": {"constraint": 74.0, "achieved": 343.879},
                        }
                    }
                )
            )
