...
class PllPhase50Tests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("320_pll_phase50")
        ...
        rtl = (ROOT / "experiments/320_pll_phase50/rtl/top.v").read_text()
        policy.validate_source_text("experiments/320_pll_phase50/rtl/top.v", rtl)
        self.assertIn('.number_of_clocks(4)', rtl)
        self.assertIn('.output_clock_frequency0("50.0 MHz")', rtl)
        self.assertIn('.output_clock_frequency3("50.0 MHz")', rtl)
        self.assertIn('.phase_shift0("0 ps")', rtl)
        self.assertIn('.phase_shift1("5000 ps")', rtl)
        self.assertIn('.phase_shift2("10000 ps")', rtl)
        self.assertIn('.phase_shift3("15000 ps")', rtl)
        self.assertIn("16'hD723", rtl)
