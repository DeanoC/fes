...
class PllFracTests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("180_pll_frac")
        self.assertEqual(policy.clock_evidence_names, ("meter.refclk",))
        self.assertEqual(dict(policy.additional_clocks_mhz), {"fractional_clock": 12.288})
        self.assertEqual(dict(policy.required_synth_cells), {"altera_pll": 1})
        rtl = (ROOT / "experiments/180_pll_frac/rtl/top.v").read_text()
        policy.validate_source_text("experiments/180_pll_frac/rtl/top.v", rtl)
        self.assertIn('.output_clock_frequency0("12.288 MHz")', rtl)
        self.assertIn('.fractional_vco_multiplier("true")', rtl)
        self.assertIn("16'hD715", rtl)
    ...
    def test_picosecond_constraint_quantization_is_accepted(self):
        ...
        generated = {"constraint": 12.28803158, "achieved": 340.716}
                ...
                json.dumps(
                    {
                        "fmax": {
                            "meter.refclk": ref,
                            "fractional_clock": {"constraint": 12.0, "achieved": 340.716},
                        }
                    }
                )
            )
