...
class PllClkenaReg2Tests(unittest.TestCase):
    def test_closed_resources_and_sources(self):
        policy = policy_for("400_pll_clkena_reg2")
        rtl = (ROOT / "experiments/400_pll_clkena_reg2/rtl/top.v").read_text()
        policy.validate_source_text("experiments/400_pll_clkena_reg2/rtl/top.v", rtl)
        self.assertIn('.ena_register_mode("double register")', rtl)
        self.assertIn('.ena_register_power_up("low")', rtl)
        self.assertIn(".enaout(enable_status)", rtl)
        self.assertIn("16'hD72B", rtl)
