# OSS primitive experiment catalog

This is the closed narrative for `experiments/NNN_*`: what each small design
proved on the open-source place-and-route lane. It is not a core recipe, not
a kit-acceptance log, and not the schedule for a later tree.

Run or extend this lane with [OSS place-and-route testing](oss-pnr.md).
Change or add a described package with [Cores](cores.md). The build contract
for both jobs is [the architecture](architecture.md).

Each experiment's own contract is `experiments/<name>/expected.md`. A note
below does not accept a core that later reuses the same primitive.
...
That pin sits on `9cbbf7353dd2b818ab73031fcf30d9993578c783`,
