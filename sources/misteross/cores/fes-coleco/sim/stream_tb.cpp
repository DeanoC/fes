...
#include "Vcoleco_application_gp.h"
#include "Vcoleco_application_gp___024root.h"
#include "verilated.h"
...
[[noreturn]] void fail(const std::string &message) {
    std::cerr << "FES Coleco application stream: " << message << '\n';
...
void run_scenario(const Json &scenario) {
    ...
    if (scenario.boolean("legacy_active", false)) {
        mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__media_open = 1;
        mailbox.dut.eval();
    }
    if (!scenario.boolean("initial_reset", true)) {
        mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__exec_reset = 0;
        mailbox.dut.eval();
    }
    ...
    require(uint32_t(mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__stream_received) == scenario.number("received"),
...
void stream_payload(Mailbox &mailbox, bool &toggle, const std::vector<uint8_t> &payload) {
    ...
    while (offset < payload.size()) {
        ...
        const uint32_t limit = offset % 2 ? 512 : 511;
        const uint32_t length = remaining > limit ? limit : remaining;
...
int main(int argc, char **argv) {
    ...
    require(argc == 2, "usage: Vcoleco_application_gp STREAM_EXCHANGES_JSON");
    ...
    for (const auto &vector : root.get("size_vectors").a) {
        ...
        if (accept)
            require(mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__stream_active, "accepted total arms");
        else
            require(!mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__stream_active && mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__stream_begin_next == 3,
...

    {
        Mailbox mailbox;
        bool toggle = false;
        for (unsigned size : {24576u, 32767u, 1u}) {
            std::vector<uint8_t> bytes(size);
            for (unsigned i = 0; i < size; ++i) bytes[i] = uint8_t(i ^ (i >> 8) ^ 0x5a);
            begin_stream(mailbox, toggle, size, crc32_ieee(bytes.data(), bytes.size()), true);
            stream_payload(mailbox, toggle, bytes);
            for (unsigned i = 0; i < size; ++i)
                require(mailbox.read_media(i) == bytes[i], "odd-offset chunks preserve every byte");
            require(mailbox.dut.media_size == size, "bounded stream reload size");
            if (size == 1)
                require(mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__media_byte0 == bytes[0] && mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__media_byte1 == 0 &&
                        mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__media_byte2 == 0, "short stream preview does not retain old bytes");
        }
    }
...

    {
        ...
        require(mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__stream_begin_next == 1, "begin next after first word");
        exchange(mailbox, toggle, 2, 0, 0, response(!toggle, false, 0),
                 "hold during stream staging");
        require(mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__stream_begin_next == 1, "hold preserves stream staging");
        require(!mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__media_open, "hold does not invent legacy open");
        exchange(mailbox, toggle, 8, 1, 0, response(!toggle, false, 0),
                 "stream begin hi after hold");
        require(mailbox.dut.rootp->coleco_application_gp__DOT__endpoint__DOT__stream_begin_next == 2, "staging advanced after hold");
        exchange(mailbox, toggle, 12, 0, 0, response(!toggle, false, 0),
                 "abort preserved stream");
    }

    std::cout << "FES Coleco application blob-stream 1.0 mailbox checks passed\n";
    return EXIT_SUCCESS;
}
