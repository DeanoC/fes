...
| --- | --- | --- | --- |
| Native idle HPS framebuffer | 640×480 BGRX, validated Linux mode and SPI enable when `IdleRecipe` declares support; production `SplashIdle()` omits it; `TransitionalMenuIdle()` still enables it for historical Menu tests | software: yes | hardware: pending |
...
| FES GP core driver | production bounded GPO/GPI identity and controls, payload-free opcode/index/toggle timeout diagnostics, fixed ADV7513-only video, normalized generation-bound input | software: yes | hardware: pending |
| FES application 1.0 | fixed video, optional stereo 48 kHz PCM audio, gamepad and blob/stream media composition, including two-port/keypad 32 KiB stream regressions; optional reset-held `fes.firmware.blob` 1.0 overlay (exact 8192 bytes, no reset release); no keyboard masquerading | software: yes | hardware: pending |
...
| FES application firmware 1.0 | protocol-2 `load_firmware` exact 8192-byte mailbox begin/data/commit while reset is held; cartridge media still releases | software: yes | hardware: pending |
