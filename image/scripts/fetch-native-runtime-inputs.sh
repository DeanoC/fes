...
if [ "$native_mode" = package-only ]; then
  [ -z "${NATIVE_RUNTIME_SYSTEMS:-}" ] || {
    printf '%s\n' 'fetch-native-runtime-inputs: package-only mode forbids NATIVE_RUNTIME_SYSTEMS' >&2
    exit 2
  }
  [ -z "${MEGADRIVE_RBF_SOURCE:-}" ] && [ -z "${MEGADRIVE_RBF_BUNDLE:-}" ] &&
    [ -z "${PONG_RBF_BUNDLE:-}" ] && [ -z "${SNES_RBF_BUNDLE:-}" ] &&
    [ -z "${NES_RBF_BUNDLE:-}" ] || {
    printf '%s\n' 'fetch-native-runtime-inputs: package-only mode forbids format-1 bundle inputs' >&2
    exit 2
  }
...
[ "$native_mode" = format1 ] || {
  printf '%s\n' 'fetch-native-runtime-inputs: native runtime mode must be format1 or package-only' >&2
  exit 2
}
