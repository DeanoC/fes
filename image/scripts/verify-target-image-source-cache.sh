...
[ "$#" -eq 2 ] || {
  printf 'usage: verify-target-image-source-cache.sh LOCK CACHE\n' >&2
  exit 2
}
...
verify_bare_repo() {
  ...
  test -f "$verify_repo_path/HEAD" && \
    test "$(git --git-dir="$verify_repo_path" rev-parse --is-bare-repository)" = true || {
      printf 'verify-target-image-source-cache: missing bare Git cache: %s.git\n' "$verify_directory" >&2
      exit 1
    }
  verify_actual=$(git --git-dir="$verify_repo_path" rev-parse refs/target-image/pinned)
  test "$verify_actual" = "$verify_expected" || {
    printf 'verify-target-image-source-cache: %s.git pin is %s, expected %s\n' \
      "$verify_directory" "$verify_actual" "$verify_expected" >&2
    exit 1
  }
  git --git-dir="$verify_repo_path" cat-file -e "$verify_expected^{commit}" || {
    printf 'verify-target-image-source-cache: %s.git lacks pinned commit object\n' "$verify_directory" >&2
    exit 1
  }
}
...
printf '%s\n' 'target image source cache provenance verified'
