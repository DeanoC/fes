...
for os in Darwin Linux; do
  ...
  for override in default explicit; do
    (
      ...
      export TEST_OS=$os SELECTOR_LOG=$fixture/selector.log FOGCAST_DIR=$fixture/platform
      : > "$SELECTOR_LOG"
...
# Host preflight uses FogCast's selector; FES copies the Linux binary for /work/bin.
fes_root=$(CDPATH='' cd -- "$repo/.." && pwd)
fogcast=${FOGCAST_DIR:-$fes_root/sources/FogCast}
for os in Darwin Linux; do
  TEST_OS=$os PATH="$fixture/path:$PATH" make -s -C "$repo" -n FOGCAST_DIR="$fogcast" \
    target-image-native-fetch > "$fixture/make-$os"
  grep -q 'target-image-lock-linux-amd64' "$fixture/make-$os"
  if [ "$os" = Darwin ]; then
    grep -Eq 'build-target-image-lock$' "$fixture/make-$os"
  elif grep -Eq 'build-target-image-lock$' "$fixture/make-$os"; then
    echo 'Linux native fetch unexpectedly builds a Darwin selector' >&2
    exit 1
  fi
done
selector=$fixture/selector
(cd "$fogcast" && go build -o "$selector" ./cmd/target-image-lock)
...
mkdir "$package"
cp "$fogcast/internal/corepackage/testdata/core-bundle-v2/manifests/valid-basic.toml" "$package/manifest.toml"
cp "$fogcast/internal/corepackage/testdata/core-bundle-v2/payloads/fes-fixture.rbf" "$package/core.rbf"
chmod 0444 "$package"/*
chmod 0555 "$package"
