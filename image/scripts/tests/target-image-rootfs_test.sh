...
fogcast_src=${FOGCAST_DIR:-$fes_root/sources/FogCast}
fogcast=$fogcast_src
...
native_agent_main=$fogcast/cmd/mister-agent/main.go
...
mkdir -p "$fixture/fogcast/bin"
: > "$fixture/fogcast/bin/mister-agent-linux-armv7"
: > "$fixture/fogcast/bin/fogcast-kit-linux-armv7"
chmod 0755 "$fixture/fogcast/bin/mister-agent-linux-armv7" \
  "$fixture/fogcast/bin/fogcast-kit-linux-armv7"
export FOGCAST_DIR=$fixture/fogcast
...
cmp "$FOGCAST_DIR/bin/fogcast-kit-linux-armv7" "$native_target/usr/sbin/fogcast-kit"
test -x "$native_target/etc/init.d/S60fogcast-kit"
grep -Fqx "fogcast_kit_sha256=$(sha256sum "$FOGCAST_DIR/bin/fogcast-kit-linux-armv7" | awk '{print $1}')" "$native_target/usr/share/mister-runtime/build-inputs"
