...
fixture=$(mktemp -d "${TMPDIR:-/tmp}/fogcast-target-image-kernel.XXXXXX")
trap 'rm -rf "$fixture"' EXIT INT TERM

build_script=$repo/scripts/build-target-kernel.sh
verify_script=$repo/scripts/verify-target-kernel.sh
test -x "$build_script"
...
grep -Fq 'ab8d809efa286413bfc78c5209f63495b36c92449767f0a8d13b1f7747b4dc9d' \
  "$repo/build/target-image-kernel-defconfig.sha256"
...
grep -Fq '"/target-image-output/kernel-work"' "$build_script"
if grep -Fq '"/target-image-output/kernel-work-$run"' "$build_script"; then
...
case "$output:${TARGET_IMAGE_KERNEL_FAKE_DIFFER:-0}" in
...
TARGET_IMAGE_KERNEL_BUILD_ONCE=$fake_build \
TARGET_IMAGE_KERNEL_OUTPUT_ROOT=$output_root \
  sh "$build_script"
...
if PATH="$fake_bin:$PATH" TARGET_IMAGE_TEST_MODE=1 \
...
expect_module_rejection() {
  if module_output=$(PATH="$fake_bin:$PATH" TARGET_IMAGE_TEST_MODE=1 \
    sh "$verify_script" --fixture "$output_root/kernel" 2>&1); then
    echo 'kernel verifier accepted an invalid module archive' >&2
    exit 1
  fi
  printf '%s\n' "$module_output" | \
    grep -Fq 'verify-target-kernel: module archive rejected:' || {
...
TARGET_IMAGE_KERNEL_FAKE_DIFFER=1 \
TARGET_IMAGE_TEST_MODE=1 \
TARGET_IMAGE_KERNEL_BUILD_ONCE=$fake_build \
TARGET_IMAGE_KERNEL_OUTPUT_ROOT=$fixture/different-output \
