...
(
  ...
  MEGADRIVE_RBF_BUNDLE=$native_bundle_path \
    make --no-print-directory -f "$repo/Makefile" \
      -o build-target-image-lock-container -o build-target-image-lock \
      FOGCAST_DIR="$fixture/fogcast" \
...
make -C "$repo" FOGCAST_DIR="$fogcast" build-target-image-lock-container
