...
sim:
	...
	@bash -c 'set -euo pipefail; source scripts/env.sh; sim_dir="$$OPEN_MISTER_ROOT/build/sim/$(EXP)"; rtl="$$OPEN_MISTER_ROOT/experiments/$(EXP)/rtl/top.v"; tb="$$OPEN_MISTER_ROOT/experiments/$(EXP)/sim/tb.cpp"; verilator="$$TOOLCHAIN_INSTALL/bin/verilator"; mkdir -p "$$sim_dir"; if [[ ! -x "$$verilator" ]]; then printf "repository-local Verilator missing: %s\\n" "$$verilator" >&2; exit 1; fi; run_logged() { local log="$$1"; shift; { printf '\''command:'\''; printf '\'' %q'\'' "$$@"; printf "\\n"; "$$@"; } >"$$log" 2>&1; }; run_logged "$$sim_dir/verilator-lint.log" "$$verilator" --lint-only --top-module top --Mdir "$$sim_dir/lint" "$$rtl"; run_logged "$$sim_dir/verilator-build.log" "$$verilator" --cc --exe --build --top-module top -GCOUNTER_BITS=4 --Mdir "$$sim_dir/obj_dir" "$$rtl" "$$tb"; run_logged "$$sim_dir/simulation.log" "$$sim_dir/obj_dir/Vtop"; cat "$$sim_dir/simulation.log"'

oss oracle compare program clean:
	$(require_exp)
	@printf 'target not implemented in this task\n' >&2
	@exit 2
