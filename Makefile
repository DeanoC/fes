...
.PHONY: help doctor build host image verify rebuild dev test check media verify-media
help:
	@printf '%s\n' 'FES: start with AGENTS.md and docs/development.md' 'make check | doctor | build | host | image | verify | rebuild | dev | media | verify-media | test' 'Default: native-integration-dev; historical: native-dev, native-source-dev' 'Source builds require QUARTUS_ROOTDIR; build does not deploy.'
