...
func registerCoreDataRoutes(mux *http.ServeMux, token string, controller DevelopmentController) {
	for _, path := range []string{"/v1/library/core/load", "/v1/library/core/data/inspect", "/v1/library/core/settings"} {
		mux.Handle(path, authenticate(token, exactMethod(http.MethodPost, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
			...
			if r.URL.RawQuery != "" || !exactContentType(r, "application/octet-stream") || len(r.TransferEncoding) != 0 || r.ContentLength < 1 || r.ContentLength > corepackage.MaxArchiveSize || len(ids) != 1 || !dataPackageIDRE.MatchString(ids[0]) {
