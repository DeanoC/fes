...
# loading build/oss/420_dsp_mul18/top.rbf. This script does not program or claim hardware.
set -eu
...
check_signature() {
    hex=$(hex_digits "$1")
    sig=${hex%????}
    if [ "$sig" != "D612" ] && [ "$sig" != "d612" ]; then
        echo "FAIL: DSP 18x18 diagnostic signature missing: $1" >&2
        exit 1
    fi
}
low16() {
...
expect_low() {
    ...
    expect=$3
    label=$4
    write_gpo "$(printf '0x%04X%04X' "$right" "$left")"
    sleep 0.02
    value=$(read_gpi)
...
expect_low 256 256 0 hex_100_times_100
expect_low 65535 2 65534 ff_times_two
expect_low 65535 65535 1 ff_times_ff
