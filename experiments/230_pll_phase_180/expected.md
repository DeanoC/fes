# 230 static 0°/180° 25 MHz PLL

`make sim EXP=230_pll_phase_180` tests ten reset/relock cycles of a dual 25 MHz
PLL with `phase_shift1("20000 ps")`. `make oss EXP=230_pll_phase_180` produces
compressed `build/oss/230_pll_phase_180/top.rbf`. It requires one PLL, one HPS
...
`CNT_PRESET=7`. Output 1 lags output 0 by 20 ns. GPO bit0 is launched on the 0°
rising edge and captured on the 180° rising edge. Each output keeps a same-clock
toggle so nextpnr reports both 25 MHz clocks in fmax (`meter.testclk` for 0° and
`phase180`). The 0° meter cannot observe analog phase accuracy. Simulation uses
two digital 25 MHz clocks 20 ns apart, not analog phase. Other shifts and
non-50% duty remain rejected.

| Register bits | Meaning |
| --- | --- |
| GPI [31:16] | Signature `0xD71A` |
...
| GPI [8] | Capture FF on the 180° clock |
...
count 0. Launch and 180° capture both follow GPO [0]. Claim the designated kit
