module top(input wire FPGA_CLK1_50);
    ...
    (* keep = "true" *) reg phase180_alive = 0;
    ...
    altera_pll #(
        ...
        .outclk({phase180, phase0}), .locked(locked)
    );
    always @(posedge phase0) begin
        launch <= gpo[0];
        phase0_alive <= ~phase0_alive;
    end
    always @(posedge phase180) begin
        capture <= launch;
        phase180_alive <= ~phase180_alive;
    end
    ...
    assign gpi = {16'hD71A, busy, done, lock_status, lost_lock, reset_sync, 1'b0, launch, capture,
