...
The FPGA exposes a mixed-width M10K true dual-port table on HPS GP:
1024-by-10 port A and 512-by-20 port B. Port A uses
the 50 MHz board clock. Port B uses an independently gated 25 MHz PLL output.
GPI signature `0xD422`; `[15:0]` are a 16-bit window of the selected
port.

Canonical 10-bit lane `a` powers up as `((a * 73) ^ (a >> 1) ^ 10'hA6)` in the
low 10 bits. Address 0 is `0xA6`. GPO bits 31/30 write ports B/A, bits 29/28
...
`MISTRAL_M10K_TDP` with `CFG_MIXED_WIDTH=1`, `CFG_ABITS`/`CFG_DBITS` 10/10
and `CFG_RD_ABITS`/`CFG_RD_DBITS` 9/20. nextpnr-mistral
...
`make sim EXP=580_m10k_tdp_mix10_20` and `make oss EXP=580_m10k_tdp_mix10_20`.
