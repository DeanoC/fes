...
namespace {
constexpr uint32_t kSignature = 0xd4260000U;
...
uint32_t command(unsigned addr, unsigned window, bool we, bool re, bool clkena) {
    return (static_cast<uint32_t>(we) << 31) | (static_cast<uint32_t>(re) << 30) |
           (static_cast<uint32_t>(clkena) << 29) | (window << 27) | (addr & 0xfu);
}
...
int main(int argc, char **argv) {
    ...
    if (!expect_read(top, 1u, initial_word(1u), "untouched addr1"))
        return EXIT_FAILURE;
    std::cout << "PASS: independent-clock 20-bit M10K SDP init and write; ACLR is kit-only\n";
    return EXIT_SUCCESS;
}
