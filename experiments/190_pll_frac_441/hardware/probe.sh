...
# loading build/oss/190_pll_frac_441/top.rbf. This script does not program or claim hardware.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55062 ]; then # 0xD716
        echo "FAIL: 11.2896 MHz PLL diagnostic signature missing: $1" >&2
        exit 1
    fi
}
set_reset() {
...
measure() {
    ...
    if [ "$1" -eq 1 ]; then
        ...
        [ "$count" -ge 924 ] && [ "$count" -le 925 ] && [ "$((status & 14336))" -eq 8192 ] || {
