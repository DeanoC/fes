// Simulation only: a digital 11.2896/50 ratio and lock delay, not analog PLL behavior.
module altera_pll #(
    parameter reference_clock_frequency = "50.0 MHz",
    parameter number_of_clocks = 1,
    parameter output_clock_frequency0 = "11.2896 MHz",
    ...
    always @(posedge refclk or posedge rst)
        if (rst) begin
            ...
            if (acc + 16'd7056 >= 16'd15625) begin
                acc <= acc + 16'd7056 - 16'd15625;
                outclk <= !outclk;
            end else
                acc <= acc + 16'd7056;
            if (acquire == 15)
