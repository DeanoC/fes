...
# loading build/oss/130_pll_dsp_40/top.rbf. This script does not program or claim hardware.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 56384 ]; then # 0xDC40
        echo "FAIL: 40 MHz PLL DSP diagnostic signature missing: $1" >&2
        exit 1
    fi
}
wait_lock() {
