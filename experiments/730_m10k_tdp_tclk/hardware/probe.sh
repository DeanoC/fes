...
# loading build/oss/730_m10k_tdp_tclk/top.rbf. This script does not program or claim hardware.
set -eu
signature=54313
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing TDP unused-clock signature $status" >&2; exit 1;
    }
    actual=$((status & 65535))
    expected=$(($1))
    [ "$actual" -eq "$expected" ] || {
...
echo 'PASS: A-port write updates memory with B clock tied off'
initial_word 1
read_word 1 "$word"
echo 'PASS: unused B port did not disturb other A-port words'
