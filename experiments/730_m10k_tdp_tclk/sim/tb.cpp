...
namespace {
constexpr uint32_t kSignature = 0xd4290000U;
...
uint32_t command(unsigned addr, bool we, bool en) {
    return (static_cast<uint32_t>(we) << 31) | (static_cast<uint32_t>(en) << 30) | (addr & 0x1ffu);
}
...
bool expect_read(Vtop &top, unsigned addr, uint32_t word, const char *label) {
    gpo(top) = command(addr, false, true);
    ticks(top, 16);
...
int main(int argc, char **argv) {
    ...
    if (!expect_read(top, 1u, initial_word(1u), "untouched addr1"))
        return EXIT_FAILURE;
    std::cout << "PASS: TDP A-port init/write with B clock tied off\n";
    return EXIT_SUCCESS;
}
