module top(input wire FPGA_CLK1_50);
    ...
    (* keep = "true" *) reg phase270_alive = 0;
    ...
    altera_pll #(
        ...
        .outclk({phase270, phase0}), .locked(locked)
    );
    always @(posedge phase0) begin
        launch <= gpo[0];
        phase0_alive <= ~phase0_alive;
    end
    always @(posedge phase270) begin
        capture <= launch;
        phase270_alive <= ~phase270_alive;
    end
    ...
    assign gpi = {16'hD71B, busy, done, lock_status, lost_lock, reset_sync, 1'b0, launch, capture,
