...
# loading build/oss/240_pll_phase_270/top.rbf. This script does not program or claim hardware.
# It checks 25 MHz and 0-to-270 capture, not analog phase accuracy.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55067 ]; then # 0xD71B
        echo "FAIL: 0/270 phase PLL diagnostic signature missing: $1" >&2;
        exit 1
    fi
}
set_reset() {
...
set_launch() {
    ...
    while :; do
        ...
        polls=$((polls + 1))
        [ "$polls" -lt 100 ] || { echo "FAIL: 0-to-270 capture timeout: $status" >&2; exit 1; }
        sleep 0.02
    done
}
measure() {
