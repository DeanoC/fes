...
int main(int argc, char **argv) {
    ...
    auto measure = [&](bool held_reset) {
        ...
        for (unsigned i = 0; i < (1U << 20) + 100; ++i) {
            ...
            if (busy && !(status & 0x8000) && ((status >> 14) & 1) == request) {
                ...
                if (held_reset) assert(count == 0);
                else assert(count >= 2047 && count <= 2049);
    ...
    for (int trial = 0; trial < 10; ++trial) {
        reset(true); measure(true);
        reset(false); measure(false);
    }
    std::cout << "PASS: 10 reset/relock cycles; held-reset zero, released 2048 +/- 1, coherent snapshots\n";
}
