...
# loading build/oss/120_pll_dsp/top.rbf. This script does not program or claim hardware.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 56336 ]; then # 0xDC10
        echo "FAIL: PLL DSP diagnostic signature missing: $1" >&2
        exit 1
    fi
}
wait_lock() {
