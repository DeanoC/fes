# 400 double-register PLL clock enable

`make sim EXP=400_pll_clkena_reg2` tests reset/relock, a low-startup gated
meter, and sampled `enaout` on a two-stage falling-edge `cyclonev_clkena`.
`make oss EXP=400_pll_clkena_reg2` produces compressed
`build/oss/400_pll_clkena_reg2/top.rbf`. It requires one PLL, one clock-enable
primitive, one HPS GP, no memory or DSP, and passing 50/25 MHz timing. Quartus
comparison is not implemented.

This experiment measures the checked PIN_V11 50→25 MHz profile with
`ena_register_mode("double register")` and power-up low. `enaout` is sampled
on the 50 MHz reference before HPS GP. GPO bit 3 drives `ena`. Status
clock-to-Q and enable setup/hold are not characterized.

| Register bits | Meaning |
| --- | --- |
| GPI [31:16] | Signature `0xD72B` |
