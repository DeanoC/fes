...
# loading build/oss/760_pll_52/top.rbf. This script does not program or claim hardware.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
write_gpo() { busybox devmem 0xFF706010 32 "$1"; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 55122 ]; then # 0xD752
        echo "FAIL: 52 MHz PLL diagnostic signature missing: $1" >&2
        exit 1
    fi
}
for trial in 1 2 3; do
    ...
    # 52 * 4096 / 50 = 4259.84; accept window-endpoint slop of one count.
    if [ "$((status & 12288))" -ne 8192 ] || [ "$count" -lt 4258 ] || [ "$count" -gt 4261 ]; then
        echo "FAIL: trial=$trial count=$count status=$status (expected 4260 +/- 2, locked, no sampled lock loss)" >&2
        exit 1
    fi
