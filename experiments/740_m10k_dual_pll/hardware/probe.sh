...
# loading build/oss/740_m10k_dual_pll/top.rbf. This script does not program or claim hardware.
set -eu
signature=54314
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing dual-PLL M10K signature $status" >&2; exit 1;
    }
    actual=$((status & 65535))
    expected=$(($1))
    [ "$actual" -eq "$expected" ] || {
...
echo 'PASS: write updates memory on the independent read clock'
initial_word 1
read_word 1 "$word"
echo 'PASS: other addresses remain initialized'
