...
# loading build/oss/840_m10k_rdw/top.rbf. This script does not program or claim hardware.
set -eu
signature=55360
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing TDP write-through signature $status" >&2; exit 1;
    }
    actual=$((status & 65535))
    expected=$(($1))
    [ "$actual" -eq "$expected" ] || {
...
echo 'PASS: same-port NEW_DATA write-through updates A1Q'
initial_word 1
read_word 1 "$word"
