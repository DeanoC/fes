...
# loading build/oss/490_m10k_sdp40/top.rbf. This script does not program or claim hardware.
set -eu
width=40
signature=54297
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing M10K SDP40 signature $status" >&2; exit 1;
    }
    actual=$((status & 65535))
    [ "$actual" -eq "$1" ] || {
...
echo "PASS: all 40 data bits initialized, 50 MHz write / 25 MHz read clocks"
