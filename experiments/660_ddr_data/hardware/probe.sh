...
# loading build/oss/660_ddr_data/top.rbf. This script does not program or claim hardware.
# It checks fabric GPI counters, not the registered pin waveform.
set -eu
read_gpi() { busybox devmem 0xFF706014 32; }
check_signature() {
    if [ "$((($1 >> 16) & 65535))" -ne 56579 ]; then # 0xDD03
        echo "FAIL: DDR fabric-data output diagnostic signature missing: $1" >&2
        exit 1
    fi
}
initial=$(read_gpi)
