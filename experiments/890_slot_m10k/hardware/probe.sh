...
# loading a composed or combined 890 slot RBF. This script does not program
# or claim hardware.
set -eu
signature=55440
...
check() {
    ...
    [ "$(((status >> 16) & 65535))" -eq "$signature" ] || {
        echo "FAIL: missing slot signature $status" >&2; exit 1;
    }
    actual=$((status & 1023))
    expected=$(($1))
    [ "$actual" -eq "$expected" ] || {
...
echo 'PASS: low slot M10K words'
for address in 255 512; do
    initial_word "$address"
    read_word "$address" "$word"
done
echo 'PASS: mid slot M10K words'
initial_word 1023
read_word 1023 "$word"
echo 'PASS: last slot M10K word'
