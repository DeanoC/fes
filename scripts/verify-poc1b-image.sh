...
usage() {
  printf 'usage: verify-poc1b-image.sh prod|dev IMAGE MANIFEST | --inside VARIANT IMAGE MANIFEST | --root-fixture VARIANT ROOT MANIFEST\n' >&2
  exit 2
}
...
EOF
  ...
  server_count=$(find "$root/usr" \( -type f -o -type l \) -name dropbear -print | wc -l | tr -d ' ')
  if [ "$variant" = prod ]; then
    test "$server_count" -eq 0 || {
      printf '%s\n' 'verify-poc1b-image: production contains Dropbear' >&2
      exit 1
    }
  else
  ...
  if find "$root" -type f \( \
    ...
    -iname '*.rbf' -o -iname '*.map' -o -name 'agent.toml' \
    -o -name '*-gdb.py' \
  \) -print -quit | grep -q .; then
    printf '%s\n' 'verify-poc1b-image: forbidden game, runtime, or debug payload found' >&2
    exit 1
  fi
  if find "$root" -type f -size -2M -exec grep -IlE '(^|[[:space:]])token[[:space:]]*=' {} + | grep -q .; then
    printf '%s\n' 'verify-poc1b-image: token assignment found' >&2
    exit 1
  fi
  ...
  if strings "$agent" | grep -Eq '(^|[[:space:]])token[[:space:]]*='; then
    printf '%s\n' 'verify-poc1b-image: agent binary contains a token assignment' >&2
    exit 1
  fi

  /bin/mkdir -p "$(dirname "$manifest")"
  manifest_tmp=$manifest.new.$$
  trap '/bin/rm -f "$manifest_tmp"' EXIT INT TERM
  (
    cd "$root"
...
case "${1:-}" in
  ...
  --inside)
    ...
    trap '/bin/rm -rf "$inspect_root"' EXIT INT TERM
    /usr/sbin/debugfs -R "rdump / $inspect_root" "$image" >/dev/null
    verify_root "$variant" "$inspect_root" "$manifest"
    /bin/rm -rf "$inspect_root"
    trap - EXIT INT TERM
    exit
    ;;
  prod|dev)
    ...
    exec "$repo/scripts/poc1b-container.sh" run \
      /work/scripts/verify-poc1b-image.sh --inside "$variant" "$image" "$manifest"
    ;;
  *) usage ;;
esac
