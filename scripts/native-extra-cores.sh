...
case "$action" in
  validate) exit 0 ;;
  count) if [ -n "$extras" ]; then echo 4; else echo 2; fi; exit 0 ;;
...
for system in $extras; do
  ...
  case "$action" in
    fetch)
      case "$system" in pong) bundle=${PONG_RBF_BUNDLE:-} ;; snes) bundle=${SNES_RBF_BUNDLE:-} ;; esac
...
case "$action" in
  install)
    if [ -z "$extras" ]; then
      rm -f "$target/usr/share/mister-runtime/cores/pong.rbf" "$target/usr/share/mister-runtime/cores/snes.rbf" "$target/usr/share/mister-runtime/selections/pong.toml" "$target/usr/share/mister-runtime/selections/snes.toml"
    fi
    ;;
  copy-records)
    if [ -z "$extras" ]; then rm -f "$target/pong.selection.toml" "$target/snes.selection.toml"; fi
    ;;
  verify-image)
    if [ -z "$extras" ]; then
