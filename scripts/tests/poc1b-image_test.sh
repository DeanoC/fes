...
POC1B_TEST_MODE=1 sh "$repo/scripts/build-poc1b-image.sh" \
  --validate-inside-path prod \
  /poc1b-output/work-1-prod \
  /work/build/output/poc1b/work-1-prod/images/rootfs.ext4
for rejected_path in \
  /poc1b-output/../work \
  /poc1b-output/work-1-dev \
  /poc1b-output/work-3-prod; do
  if POC1B_TEST_MODE=1 sh "$repo/scripts/build-poc1b-image.sh" \
    --validate-inside-path prod "$rejected_path" \
    /work/build/output/poc1b/work-1-prod/images/rootfs.ext4 >/dev/null 2>&1; then
...
if POC1B_TEST_MODE=1 sh "$repo/scripts/build-poc1b-image.sh" \
  --validate-inside-path prod \
  /poc1b-output/work-1-prod \
  /work/build/output/poc1b/work-2-prod/images/rootfs.ext4 >/dev/null 2>&1; then
...
grep -Fq 'test mode is required for --promote-existing' \
  "$repo/scripts/build-poc1b-image.sh"
grep -Fq 'test mode is required for POC1B_BUILD_ONCE' \
  "$repo/scripts/build-poc1b-image.sh"
...
verify_fixture() {
  ...
  PATH="$fake_bin:$PATH" POC1B_TEST_MODE=1 \
    sh "$repo/scripts/verify-poc1b-image.sh" --root-fixture \
...
if POC1B_TEST_MODE=1 \
  sh "$repo/scripts/qemu-smoke-poc1b.sh" --verify-log prod "$smoke_log.no-wait" >/dev/null 2>&1; then
