...
grep -Fq '[ ! -f /media/fat/mister-remote/agent.toml ]' "$agent"
...
grep -Fq 'agent_binary=/media/fat/mister-remote/mister-agent' "$agent"
grep -Fq '/usr/sbin/mister-supervise mister-agent "$agent_binary" --config /media/fat/mister-remote/agent.toml' "$agent"
