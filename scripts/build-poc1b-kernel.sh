...
usage() {
  printf 'usage: build-poc1b-kernel.sh | --inside OUTPUT EXPORT EPOCH\n' >&2
  exit 2
}
...
validate_inside_paths() {
  ...
  case "$validate_output:$validate_export" in
    /poc1b-output/kernel-work:/work/build/output/poc1b/kernel-work-1|\
    /poc1b-output/kernel-work:/work/build/output/poc1b/kernel-work-2) : ;;
    *)
      printf 'build-poc1b-kernel: unsafe or mismatched paths: %s -> %s\n' \
...
verify_materialized_source() {
  verify_bare=$1
  verify_kernel=$2
  verify_index=/poc1b-output/kernel-source.index
  ...
  /bin/rm -f "$verify_index"
  test -z "$verify_diff" || {
    printf '%s\n' 'build-poc1b-kernel: materialized source differs from pinned commit' >&2
...
inside_build() {
  ...
  test "$(/usr/bin/id -u)" -ne 0 || {
    printf '%s\n' 'build-poc1b-kernel: refusing to build as root' >&2
    exit 1
  }
  ...
  locked_defconfig=$(tr -d '[:space:]' < /work/build/poc1b-kernel-defconfig.sha256)
  test "$locked_defconfig" = "$defconfig_sha256"
  bare=/work/build/cache/poc1b/linux-kernel.git
  test "$(git --git-dir="$bare" rev-parse refs/poc1b/pinned)" = "$kernel_commit"
  ...
  test "$bare_defconfig" = "$defconfig_sha256" || {
    printf '%s\n' 'build-poc1b-kernel: committed MiSTer_defconfig digest differs from lock' >&2
    exit 1
  }

  kernel=/poc1b-output/kernel-source
  /bin/rm -rf "$kernel"
  /bin/mkdir -p "$kernel"
  ...
  cross=/poc1b-output/work-2-prod/host/bin/arm-buildroot-linux-gnueabihf-
  toolchain_host=/poc1b-output/work-2-prod/host
  test -x "${cross}gcc" || {
    printf '%s\n' 'build-poc1b-kernel: canonical production Buildroot toolchain is missing' >&2
    exit 1
  }
  ...
  export KBUILD_BUILD_USER=mister-remote
  export KBUILD_BUILD_HOST=poc1b
  ...
  test "$actual_release" = "$kernel_release" || {
    printf 'build-poc1b-kernel: release is %s, expected %s\n' \
  ...
  cmp "$inside_output/.config.after-defconfig" "$inside_output/.config" || {
    printf '%s\n' 'build-poc1b-kernel: build changed generated MiSTer_defconfig' >&2
    exit 1
  }
...
output_root=${POC1B_KERNEL_OUTPUT_ROOT:-$repo/build/output/poc1b}
if [ "${POC1B_TEST_MODE:-0}" != 1 ]; then
  test "$output_root" = "$repo/build/output/poc1b" || {
    printf '%s\n' 'build-poc1b-kernel: output override requires test mode' >&2
    exit 2
  }
fi
case "$output_root" in
  /*) : ;;
  *)
    printf '%s\n' 'build-poc1b-kernel: output root must be absolute' >&2
    exit 2
    ;;
esac
if [ -n "${POC1B_KERNEL_BUILD_ONCE:-}" ] && [ "${POC1B_TEST_MODE:-0}" != 1 ]; then
  printf '%s\n' 'build-poc1b-kernel: fake builder requires test mode' >&2
  exit 2
fi
...
for run in 1 2; do
  ...
  /bin/rm -rf "$work"
  if [ -n "${POC1B_KERNEL_BUILD_ONCE:-}" ]; then
    "$POC1B_KERNEL_BUILD_ONCE" "$work" "$epoch"
    write_manifest "$work"
  else
    "$repo/scripts/poc1b-container.sh" run \
      /work/scripts/build-poc1b-kernel.sh --inside \
      "/poc1b-output/kernel-work" \
      "/work/build/output/poc1b/kernel-work-$run" "$epoch"
  fi
done
...
for artifact in zImage MiSTer.dtb zImage_dtb modules.tar.gz config manifest.toml; do
  ...
  cmp "$output_root/kernel-work-1/$artifact" "$output_root/kernel-work-2/$artifact" || {
    printf 'build-poc1b-kernel: %s is not reproducible\n' "$artifact" >&2
    exit 1
  }
done
