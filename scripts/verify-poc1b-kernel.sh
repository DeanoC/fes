...
usage() {
  printf 'usage: verify-poc1b-kernel.sh DIR | --inside DIR | --fixture DIR\n' >&2
  exit 2
}
...
verify_kernel() {
  ...
  for artifact in zImage MiSTer.dtb zImage_dtb modules.tar.gz config manifest.toml; do
    test -s "$dir/$artifact" || {
      printf 'verify-poc1b-kernel: missing or empty artifact: %s\n' "$artifact" >&2
      exit 1
    }
  done

  combined=$(mktemp "${TMPDIR:-/tmp}/mister-remote-zimage-dtb.XXXXXX")
  names=$(mktemp "${TMPDIR:-/tmp}/mister-remote-kernel-names.XXXXXX")
  trap 'rm -f "$combined" "$names"' EXIT INT TERM
  cat "$dir/zImage" "$dir/MiSTer.dtb" > "$combined"
  cmp "$combined" "$dir/zImage_dtb" || {
    printf '%s\n' 'verify-poc1b-kernel: DTB is not appended exactly once' >&2
    exit 1
  }
  ...
  for symbol in \
    ...
    grep -Fxq "$symbol=y" "$dir/config" || {
      printf 'verify-poc1b-kernel: required config is absent: %s=y\n' "$symbol" >&2
      exit 1
    }
  done
...
zImage_dtb'
  test "$(cat "$names")" = "$expected_names" || {
    printf '%s\n' 'verify-poc1b-kernel: manifest artifact set is not exact' >&2
    exit 1
  }
  for artifact in zImage MiSTer.dtb zImage_dtb modules.tar.gz config; do
    ...
    test "$recorded_sha" = "$(sha256_file "$dir/$artifact")" || {
      printf 'verify-poc1b-kernel: digest mismatch: %s\n' "$artifact" >&2
      exit 1
    }
    test "$recorded_size" = "$(file_size "$dir/$artifact")" || {
      printf 'verify-poc1b-kernel: size mismatch: %s\n' "$artifact" >&2
      exit 1
    }
  done
  rm -f "$combined" "$names"
  trap - EXIT INT TERM
  printf 'POC 1B kernel verified: %s\n' "$(sha256_file "$dir/zImage_dtb")"
}
...
case "${1:-}" in
  --fixture)
    ...
    test "${POC1B_TEST_MODE:-0}" = 1 || {
      printf '%s\n' 'verify-poc1b-kernel: fixtures require test mode' >&2
      exit 2
    }
    verify_kernel "$2"
    ;;
  --inside)
    [ "$#" -eq 2 ] || usage
    test "$2" = /work/build/output/poc1b/kernel || {
      printf '%s\n' 'verify-poc1b-kernel: unsafe container artifact path' >&2
      exit 2
    }
    verify_kernel "$2"
    ;;
  *)
    [ "$#" -eq 1 ] || usage
    expected=$repo/build/output/poc1b/kernel
    requested=$(CDPATH='' cd -- "$1" 2>/dev/null && pwd -P) || {
      printf 'verify-poc1b-kernel: artifact directory does not exist: %s\n' "$1" >&2
      exit 1
    }
    test "$requested" = "$expected" || {
      printf '%s\n' 'verify-poc1b-kernel: only the canonical artifact directory may be verified' >&2
      exit 2
    }
    exec "$repo/scripts/poc1b-container.sh" run \
      /work/scripts/verify-poc1b-kernel.sh --inside \
      /work/build/output/poc1b/kernel
    ;;
esac
